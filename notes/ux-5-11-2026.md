...
flowchart TB
    ...
    subgraph LCPage["Overlay on leetcode.com (content script)"]
        ...
        L2["After paste detected:<br/>nothing visible to student"]:::silent
    end

    subgraph Email["Email / Teams ping (TAs + instructor only)"]
        E1["Falling-behind alert<br/>'Bryce W. — 0 problems this week'"]:::intrusive
        E2["Copy-paste alert<br/>'Andrew R. pasted 3 solutions today'"]:::intrusive
    end
...
| Copy-paste detection | Silent for student + **proactive email/Teams alert to TAs/instructor** | **95%** | Decision 5/11: pure silent isn't enough — nobody would proactively check the dashboard. Push it to the humans so they can reach out personally via Teams. Same "human touch" model as falling-behind alerts. |
...
6. ~~Copy-paste — truly silent, or one gentle nudge?~~ **Resolved 5/11:** silent for the student, but the tool **emails / Teams-pings the instructor + TAs** when it detects pasting. The instructor reaches out via Teams personally. Reasoning: pure silent doesn't trigger action because no one will check the dashboard on their own; we'd "waste a month" before noticing.

---

## Related infrastructure to reuse (future)

- **Email-sending pipeline.** The instructor has a previous BYU Pathway project that already sends automated emails. When we get to building the falling-behind / copy-paste alert pipeline, reuse that pattern rather than building from scratch. Instructor offered to walk Jack through it. (Captured for a future session — not blocking initial Chrome extension work.)
