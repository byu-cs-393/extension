...
// ---- LeetCode sessions (keystroke capture) -----------------------------
//
// Reads what keystroke-tracker.js has been writing and runs it through
// keystroke-analysis.js. Two-stage on purpose:
//
...
async function fetchKeystrokeSessions(netID) {
...
async function loadSessionSummary(netID, session) {
...
function renderKeystrokeSection(body, netID, sessions) {
  const h2 = document.createElement("h2");
  ...
  const rows = sessions.map((session) => {
    const row = renderSessionRow(netID, session);
...
// expand uses.
function renderSessionRow(netID, session) {
  ...
  async function analyze() {
    ...
    loading = (async () => {
      const summary = await loadSessionSummary(netID, session);
